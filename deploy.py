...
def getDeployments():
  ...
  for env in environments:
    ...
    for rev in revisions:
      ...
      status = {'environment' : envName, 'revision' : revNum,
                'basePath' : basePath,
                'state' : state}
      if error != None:
        status['error'] = error
...
if UserPW == None or Directory == None or Environment == None or Name == None or Organization == None:
  print 'Usage: deploy -n [name] -d [directory name] -e [environment]'
  print '              -u [username:password] -o [organization]'
  print '              [-p [base path] -h [apigee API url] -i -z]'
  print '  base path defaults to "/"'
  print '  Apigee URL defaults to "https://api.enterprise.apigee.com"'
  print '  -i denotes to import only and not actually deploy'
  sys.exit(1)
...
uri =  '/v1/organizations/%s/apis?action=import&name=%s' \
             % (Organization, Name)
resp = httpCall('POST', uri, hdrs, tf.getvalue())
...
revision = int(deployment['APIProxy']['@revision'])
